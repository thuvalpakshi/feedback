...
if (elgg_instanceof($page_owner, 'user')) {
	$url_segment = 'feedback/archive/' . $page_owner->username;
} else {
	$url_segment = 'feedback/group/' . $page_owner->getGUID() . '/archive';
}

// This is a limitation of the URL schema.
if ($page_owner && $vars['page'] != 'friends') {
	$dates = get_entity_dates('object', 'feedback', $page_owner->getGUID());
	
	if ($dates) {
		$title = elgg_echo('feedback:archives');
		$content = '<ul class="feedback-archives">';
